...
| Figura | Página |
|:---|---:|
| Figura 1 — PAM50, centroides moleculares e a pergunta do CorePAM | 23 |
| Figura 2 — Subtipos intrínsecos e correlatos clínicos do câncer de mama | 24 |
| Figura 3 — Linha do tempo das assinaturas gênicas em câncer de mama | 26 |
| Figura 4 — Desenho do estudo CorePAM. Derivação na coorte SCAN-B (N = 3 069)… | 35 |
| Figura 5 — Fluxograma das coortes do estudo | 41 |
| Figura 6 — Esquema de derivação do CorePAM | 46 |
| Figura 7 — Fronteira de Pareto: C-index out-of-fold em função do número de genes (graus… | 55 |
| Figura 8 — Pesos dos 24 genes CorePAM (gráfico lollipop) | 58 |
| Figura 9 — Estabilidade bootstrap da seleção de genes (B = 200 reajustes no SCAN-B) | 60 |
| Figura 10 — Análise de sensibilidade leave-one-gene-out | 62 |
| Figura 11 — Curvas de Kaplan-Meier nas cinco coortes (painel múltiplo) | 66 |
| Figura 12 — Curva de Kaplan-Meier na coorte de treinamento SCAN-B (N = 3 069; sobrevida… | 68 |
| Figura 13 — Curva de Kaplan-Meier na coorte TCGA-BRCA (N = 1 072; RNA-seq; sobrevida… | 69 |
| Figura 14 — Curva de Kaplan-Meier na coorte METABRIC (N = 1 978; microarray Illumina… | 70 |
| Figura 15 — Curva de Kaplan-Meier na coorte GSE20685 (N = 327; microarray Affymetrix… | 71 |
| Figura 16 — Forest plot da meta-análise de sobrevida (HR por 1-DP, K = 4 coortes… | 72 |
| Figura 17 — Delta-C-index incremental do CorePAM sobre o modelo clínico CORE-A | 75 |
| Figura 18 — Curvas de calibração (risco predito vs observado) em quatro coortes | 77 |
| Figura 19 — Correlação entre escores intra-coorte e z-score congelado (parâmetros SCAN-B)… | 80 |
| Figura 20 — Análise de curva de decisão (DCA) — quatro coortes de sobrevida | 82 |
| Figura 21 — PCA forense da matriz de expressão METABRIC | 84 |
| Figura 22 — Forest plot de subgrupos por status de receptor de estrogênio (ER-positivo… | 86 |
| Figura 23 — Forest plot de pCR (OR por 1-DP, K = 4 coortes primárias) | 88 |
| Figura 24 — Curvas ROC para predição de pCR em cada coorte neoadjuvante | 89 |
| Figura 25 — Distribuição do escore CorePAM por status de pCR em cada coorte neoadjuvante | 90 |
| Figura 26 — Taxa de pCR por quartil do escore CorePAM em cada coorte neoadjuvante | 91 |
| Figura 27 — Análise de curva de decisão para predição de pCR nas quatro coortes… | 92 |
| Figura 28 — Comparação head-to-head CorePAM (24 genes) vs PAM50-full penalizado (49… | 95 |
| Figura 29 — Comparação de C-index: CorePAM vs ROR-S vs OncotypeDX-RS (implementações… | 97 |
...
| Tabela | Página |
|:---|---:|
| Tabela 1 — Mapa dos blocos de coortes e das perguntas metodológicas do estudo | 30 |
| Tabela 2 — Papel de cada coorte na construção e validação do CorePAM | 30 |
| Tabela 3 — Características das coortes de sobrevida | 39 |
| Tabela 4 — Características das coortes neoadjuvantes (pCR) | 41 |
| Tabela 5 — Parâmetros analíticos congelados antes da análise | 42 |
| Tabela 6 — Cobertura de genes CorePAM por coorte de validação | 44 |
| Tabela 7 — Lista completa dos 24 genes CorePAM com pesos elastic-net | 56 |
| Tabela 8 — Desempenho prognóstico do CorePAM por coorte | 64 |
| Tabela 9 — Meta-análise de efeitos aleatórios — sobrevida (K = 4) | 72 |
| Tabela 10 — C-index incremental do CorePAM sobre o modelo CORE-A | 74 |
| Tabela 11 — Modelo clínico expandido com estadiamento anatômico | 78 |
| Tabela 12 — Modelo clínico completo (6 variáveis) com e sem CorePAM | 78 |
| Tabela 13 — Análise de sensibilidade do z-score congelado | 78 |
| Tabela 14 — Estatística C de Uno e AUC tempo-dependente | 81 |
| Tabela 15 — Calibração formal por coorte | 81 |
| Tabela 16 — Análise de subgrupo por status de RE em SCAN-B (N = 2 870; OS) e METABRIC (N… | 85 |
| Tabela 17 — Predição de pCR por coorte neoadjuvante | 87 |
| Tabela 18 — Comparação head-to-head: CorePAM (24 genes) vs comparador PAM50-full… | 94 |
| Tabela 19 — Comparação com ROR-S e OncotypeDX-RS (implementações genefu) | 96 |
| Tabela 20 — Organização do pipeline computacional por blocos | 120 |
| Tabela 21 — Lista completa dos scripts do pipeline | 120 |
...
|:---|---:|
| **1 INTRODUÇÃO** | 20 |
| 1.1 Epidemiologia e carga global da doença | 20 |
| 1.2 Heterogeneidade tumoral e justificativa da estratificação molecular | 21 |
| 1.3 Subtipos moleculares intrínsecos e o PAM50 | 21 |
| 1.4 Assinaturas gênicas: panorama atual e evidência clínica | 25 |
| 1.5 Assinaturas gênicas, plataformas e transportabilidade | 26 |
| 1.6 Coortes públicas de câncer de mama | 29 |
| 1.7 A pergunta central | 31 |
| **2 OBJETIVOS** | 33 |
| 2.1 Objetivo geral | 33 |
| 2.2 Objetivos específicos | 33 |
| **3 MÉTODO** | 34 |
| 3.1 Delineamento do estudo e aspectos éticos | 34 |
| 3.1.1 Tipo de estudo | 34 |
| 3.1.2 Aspectos éticos | 37 |
| 3.1.3 Princípios de validação externa | 37 |
| 3.1.4 Disponibilidade de dados e código | 38 |
| 3.1.5 Uso de modelos de linguagem de grande escala | 38 |
| 3.2 Coortes do estudo | 38 |
| 3.3 Congelamento analítico (analytical freeze) | 42 |
| 3.3.1 Margem de não inferioridade (Delta-C-index = 0,010) | 43 |
| 3.3.2 Atribuição determinística de folds (SHA-256) | 43 |
| 3.3.3 Estratificação por evento | 43 |
| 3.4 Pré-processamento de expressão gênica | 44 |
| 3.4.1 Coortes RNA-seq (SCAN-B, TCGA-BRCA) | 44 |
| 3.4.2 Coortes microarray (METABRIC, GSE20685, GSE1456, GSE25066, GSE20194, | 44 |
| 3.5 Derivação do CorePAM | 46 |
| 3.5.1 Regressão de Cox elastic-net: justificativa e parâmetros | 47 |
| 3.5.2 Validação cruzada out-of-fold | 47 |
| 3.5.3 Critério de seleção: não inferioridade e fronteira de Pareto | 48 |
| 3.5.4 Análises de estabilidade da seleção | 48 |
| 3.6 Cálculo do escore | 48 |
| 3.6.1 Pontuação congelada (single-sample scoring) | 49 |
| 3.7 Análise de sobrevida | 49 |
| 3.7.1 Modelos de Cox | 49 |
| 3.7.2 Métricas de discriminação | 50 |
| 3.7.3 Curvas de Kaplan-Meier | 50 |
| 3.7.4 Valor incremental: Delta-C-index | 50 |
| 3.7.5 Calibração | 51 |
| 3.7.6 Análise de curva de decisão (DCA) | 51 |
| 3.7.7 Modelos expandidos com estadiamento | 52 |
| 3.8 Meta-análise | 52 |
| 3.9 Comparações head-to-head | 52 |
| **4 RESULTADOS** | 54 |
| 4.1 Derivação do CorePAM: os 24 genes | 54 |
| 4.2 Validação prognóstica externa | 64 |
| 4.3 Meta-análise de sobrevida | 72 |
| 4.4 Valor incremental sobre preditores clínicos | 74 |
| 4.5 Independência do estadiamento clínico-patológico | 78 |
| 4.6 Análises de sensibilidade | 78 |
| 4.7 Análise de subgrupo por status de RE | 85 |
| 4.8 Resposta patológica completa (pCR) | 87 |
| 4.9 Comparação head-to-head com PAM50-full | 94 |
| 4.10 Comparação com assinaturas estabelecidas | 96 |
| **5 DISCUSSÃO** | 98 |
| 5.1 O CorePAM no contexto da medicina de precisão em oncologia | 98 |
| 5.2 Derivação reprodutível: inovação metodológica | 99 |
| 5.3 Coerência biológica: por que esses 24 genes? | 100 |
| 5.4 Transportabilidade cross-plataforma | 101 |
| 5.5 O paradoxo de menos genes, mais informação | 102 |
| 5.6 A questão do z-score: coorte vs amostra única | 103 |
| 5.7 O sinal em doença ER-positiva | 103 |
| 5.8 A ponte entre prognóstico e predição de pCR | 104 |
| 5.9 Calibração: boa discriminação não garante probabilidades corretas | 105 |
| 5.10 Reflexões metodológicas para estudos prognósticos futuros | 105 |
| 5.11 O caminho até a clínica: o que falta? | 106 |
| 5.12 Relevância para o contexto brasileiro e o SUS | 106 |
| 5.13 Limitações | 107 |
| 5.14 Perspectivas e estudos futuros | 108 |
| **6 CONCLUSÃO** | 109 |
| **REFERÊNCIAS BIBLIOGRÁFICAS** | 110 |
| **APÊNDICE A — Pipeline computacional** | 119 |
| **APÊNDICE B — Artigo publicado no Breast Cancer Research** | 130 |
...
- **Validação externa:** realizada em quatro coortes completamente independentes — uma segunda coorte de *RNA-seq* (TCGA-BRCA) e três de *microarray* —, sem reajuste de qualquer parâmetro. Os pesos congelados do SCAN-B são aplicados diretamente aos *z-scores* calculados intra-coorte em cada conjunto de validação.
...
| 19 | CENPF | +0,0150 | Risco (+) | Proliferação / centrômero |
...
: Desempenho prognóstico do CorePAM por coorte. SCAN-B é apresentada apenas como referência interna; seu *C-index* na tabela (0,698) é aparente (*in-sample*) e não deve ser comparado diretamente ao *C-index* *out-of-fold* da derivação (0,670) nem às coortes externas. Nas coortes externas, os pesos já estavam congelados. Em GSE1456, o modelo ajustado CORE-A não foi estimado por ausência de covariáveis clínicas suficientes. {#tbl-survival-performance tbl-colwidths="[24,14,24,19,19]"}
...
|:--------------------|:-----------------|------------:|:----------:|:------------:|
| Primária (REML)     | 1,37 (1,24–1,52) | 1,6 × 10⁻⁹  |   38,2%    |    0,0067    |
...
| **Agrupado — efeitos aleatórios (K=4)** | **697** | | **1,69 (1,39-2,05)** | **1,9 × 10^-7^** | |
...
```{r fig-pcr-roc}
#| label: fig-pcr-roc
#| fig-cap: "Curvas ROC para predição de pCR em cada coorte neoadjuvante. AUC variou de 0,576 (GSE25066) a 0,716 (GSE32646), indicando capacidade discriminativa baixa a moderada — ainda assim notável, considerando que o escore foi derivado para sobrevida, não para pCR."
#| out-width: "15.8cm"
#| fig-align: center
