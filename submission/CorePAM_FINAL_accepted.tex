...
\textbf{Conclusions:} CorePAM---a 24-gene PAM50-derived score---met the pre-specified $\Delta$C-index non-inferiority criterion relative to a 50-gene Cox elastic-net comparator in derivation and retained prognostic discrimination across four external RNA-seq and microarray cohorts, remained significant after anatomical staging adjustment, and was associated with pCR in secondary neoadjuvant analyses. This reduction from 50 to 24 genes may simplify future assay development, although platform-specific analytical validation is required before clinical deployment.
}
...
The PAM50 intrinsic subtype gene expression test classifies breast tumours into five molecular subtypes---Luminal~A, Luminal~B, HER2-enriched, Basal-like, and Normal-like~\cite{perou2000,sorlie2001}---and underpins the Prosigna prognostic assay~\cite{parker2009}. Its prognostic utility is well established across multiple independent cohorts and it is widely used for adjuvant chemotherapy decision-making in hormone receptor-positive, HER2-negative early breast cancer~\cite{cardoso2016,sparano2018}.
...
\item \textit{GSE20685} (N = 327)~\cite{kao2011}: Affymetrix HG-U133 Plus 2.0 microarray (GPL570); primary endpoint OS.
...
\begin{itemize}
\item GSE25066 (N = 508)~\cite{hatzis2011}, GSE20194 (N = 278)~\cite{maqc2010}, GSE32646 (N = 115)~\cite{miyake2012}, I-SPY1/GSE22226 (N = 149)~\cite{esserman2012}: GEO microarray cohorts with pCR data from neoadjuvant chemotherapy trials. After applying eligibility criteria (available pCR outcome and expression data for CorePAM genes), 697 samples were included in the primary pCR analysis.
...
\textit{Microarray cohorts (METABRIC, GSE20685, GSE1456):} Log2-scaled intensities from GEO Series Matrix files (pre-processed by the original data submitters) were used without additional normalisation. METABRIC identifiers were mapped via org.Hs.eg.db; GSE20685 (Affymetrix HG-U133 Plus 2.0, GPL570) via hgu133plus2.db; GSE1456 (Affymetrix HGU133A/B, GPL96) via hgu133a.db. GSE20685 achieves full CorePAM coverage (24/24 = 100\%). For GSE1456, the HGU133A platform ceiling is 44/50 PAM50 genes (88\%, above the 80\% minimum), with three CorePAM genes absent (GPR160, CXXC5, KRT17; 21/24 = 87.5\%). METABRIC ER status was obtained from the cBioPortal clinical file (ER\_IHC column).

\textit{Score completeness:} When fewer than 24 CorePAM genes are available (e.g., GSE1456: 21/24 genes, 87.5\%), the score is computed over available genes only, with the denominator renormalised accordingly. Gene coverage is reported per cohort (Table~\ref{tab:coverage}); the pre-specified minimum of 80\% was met in all cases. Sensitivity to missing genes is further assessed in a leave-one-gene-out analysis (Additional file~1: Figure~S1).
...
GSE20685 & Affymetrix HG-U133 Plus 2.0 & 24/24 & 100 & --- \\
...
\textbf{Clinical baseline model scope:} CORE-A was pre-specified as the minimum clinical baseline (age and ER status) for consistency across cohorts, avoiding post-hoc covariate selection decisions. T-stage and nodal status were reserved for exploratory sensitivity analyses (Additional file~3: Table~S1), as their harmonisation quality varied across public data sources and their inclusion in the primary model would have introduced cohort-specific modelling decisions incompatible with a uniform validation framework.
...
Comprehensive QC comprised: (i) off-diagonal score correlation between cohorts via percentile-based Pearson $r$ (Additional file~4: Figure~S3); (ii) forensic PCA of METABRIC expression for batch effects and outliers (Additional file~5: Figure~S4); (iii) schema/range hard-checks on all analysis-ready files; (iv) automated anti-hardcoding assertions verifying all reported values against source CSV/JSON files.
...
The 24 genes span luminal differentiation (ESR1, PGR, NAT1), proliferation (MYBL2, PTTG1, EXO1, MYC), basal phenotype (KRT5, KRT17, FOXC1), and HER2-enriched markers (ERBB2, GRB7, MIA), with full gene list in Additional file~7: Table~S2. The Pareto frontier (Additional file~8: Figure~S6) shows diminishing returns beyond approximately 15 genes, with the non-inferiority boundary crossed at df = 24. Bootstrap resampling ($B = 200$) confirmed that 15 of 24 CorePAM genes were selected in at least 70\% of resamples, with a minimum selection frequency of 35.5\% (Additional file~16: Figure~S10). To assess whether gene reduction compromised external discrimination, we compared CorePAM directly against the PAM50-full elastic-net model trained on all 50 candidate genes (49 non-zero coefficients at the selected $\lambda$) in all four validation cohorts under both cohort-standardised and frozen-reference scoring. In paired bootstrap analyses, differences were small in TCGA-BRCA and favoured CorePAM in the remaining three cohorts (Additional file~18: Table~S8).

\subsection{Prognostic validation}\label{subsec:results-validation}

The CorePAM score was independently associated with survival in all four external validation cohorts (Table~\ref{tab:survival}). In TCGA-BRCA (RNA-seq), HR per 1~SD = 1.20 (95\% CI 1.04--1.40; $p = 0.016$). In METABRIC (DSS endpoint; Illumina microarray), HR = 1.41 (95\% CI 1.31--1.52; $p < 0.001$). In GSE20685 (Affymetrix microarray), HR = 1.40 (95\% CI 1.12--1.75; $p = 0.003$). In GSE1456 (Stockholm; Affymetrix microarray), HR = 1.71 (95\% CI 1.23--2.38; $p = 0.001$). Kaplan--Meier curves stratified at the intra-cohort median score are shown in Figure~\ref{fig:km-multipanel}.
...
GSE20685 & OS & 327 & 83 & 1.40 (1.12--1.75) & 1.40 (1.12--1.75) & 0.626 (0.565--0.684) \\
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth,height=0.92\textheight,keepaspectratio]{figures/Fig2_KM_MultiPanel_EN.png}
\caption{Kaplan--Meier survival curves across all five cohorts, stratified at the intra-cohort median CorePAM score (High vs Low risk). \textbf{A.}~SCAN-B training cohort (OS; N = 3,069). \textbf{B.}~TCGA-BRCA (OS; N = 1,072). \textbf{C.}~METABRIC (DSS; N = 1,978). \textbf{D.}~GSE20685 (OS; N = 327). \textbf{E.}~GSE1456 Stockholm (OS; N = 159). Each panel shows the dichotomised Cox HR with 95\% CI and log-rank $p$-value. Shading: 95\% CI. Cutoff: intra-cohort median (pre-specified, not optimised).}\label{fig:km-multipanel}
\end{figure}

TCGA-BRCA has shorter follow-up (median 32 months), attenuating the HR; a 24-month sensitivity analysis yielded HR = 1.64 (95\% CI 1.24--2.16; $p = 4.7 \times 10^{-4}$; Additional file~9: Figure~S7).

Multivariate Cox regression including the CORE-A clinical model (age and ER status where available) yielded adjusted HRs of 1.27 (TCGA-BRCA), 1.36 (METABRIC), and 1.40 (GSE20685), indicating that CorePAM provides prognostic information independent of clinical covariates. GSE1456 lacked clinical covariates for CORE-A adjustment; the univariate HR of 1.71 nonetheless confirms cross-platform generalizability on an independent Affymetrix dataset. The training cohort (SCAN-B; panel~A of Figure~\ref{fig:km-multipanel}) showed an in-sample C-index of 0.698 (OOF C-index = 0.670).

\subsection{Meta-analysis}\label{subsec:results-meta}

Random-effects meta-analysis across four validation cohorts yielded a pooled HR of 1.37 (95\% CI 1.24--1.52; $p = 1.6 \times 10^{-9}$) with moderate heterogeneity ($I^2$ = 38.2\%; Figure~\ref{fig:forest-os}). The Hartung--Knapp sensitivity analysis confirmed significance under conservative inference (Table~\ref{tab:meta}). OS-harmonised and leave-one-out sensitivity analyses maintained consistent positive direction.
...
\midrule
Primary RE (REML) & 1.37 (1.24--1.52) & $1.6 \times 10^{-9}$ & 38.2\% \\
...
GSE20685 & age & 327 & 0.536 & 0.630 & +0.094 (0.021--0.175) \\
\botrule
\end{tabular}
\end{table}
...
\textbf{Frozen z-score (single-sample scoring feasibility).} When the SCAN-B training cohort mean and standard deviation per gene were frozen and applied to external validation cohorts---without any cohort-level re-standardisation---the C-index was preserved in RNA-seq and array cohorts sharing a similar dynamic range: TCGA-BRCA $\Delta$C = +0.0002 (Pearson $r$ = 0.999); METABRIC $\Delta$C = +0.0007 ($r$ = 0.958). In the Affymetrix microarray cohorts, which have a markedly different intensity distribution from the RNA-seq training reference, discrimination was attenuated: GSE20685 $\Delta$C = $-$0.036 ($r$ = 0.923); GSE1456 $\Delta$C = $-$0.040 ($r$ = 0.916). These results suggest that frozen single-sample scoring is feasible when the assay platform is compatible with the training reference, but that platform-specific recalibration would be needed for Affymetrix-based deployment (Additional file~12: Table~S3).

\textbf{Uno's C-statistic and time-dependent AUC.} IPCW-based discrimination metrics confirmed the Harrell's C-index findings: Uno's C ranged from 0.668 (GSE20685) to 0.681 (METABRIC); time-dependent AUC at the cohort-specific horizon ranged from 0.675 (GSE20685, 60m) to 0.695 (METABRIC, 60m). These concordance estimates are robust to heterogeneous censoring (Additional file~13: Table~S4).

\textbf{Formal calibration.} Logistic recalibration at the cohort-specific horizon yielded calibration slopes ranging from 0.96 (SCAN-B, near-ideal) to 2.74 (TCGA-BRCA), with corresponding intercepts of 0.56 (SCAN-B) to 5.40 (TCGA-BRCA). The training cohort showed a near-ideal calibration slope ($\beta$ = 0.96), though the non-zero intercept ($\alpha$ = 0.56) indicates moderate systematic underprediction of absolute risk. External validation cohorts showed miscalibration that was mild in METABRIC ($\beta$ = 1.77) and GSE20685 ($\beta$ = 1.57) but substantial in TCGA-BRCA ($\beta$ = 2.74), the latter consistent with its short follow-up and low event rate at the 24-month horizon (Additional file~14: Table~S5).
...
CorePAM is a data-driven 24-gene reduction of PAM50 selected within a pre-specified $\Delta$C-index framework relative to a full 50-gene elastic-net comparator. In newly added direct paired external comparisons, discrimination was similar in TCGA-BRCA and favoured the reduced model in METABRIC, GSE20685, and GSE1456 under both cohort-standardised and frozen-reference scoring (Additional file~18: Table~S8). These findings support the robustness of the reduced model within the present elastic-net framework. However, external calibration and frozen-reference single-sample performance remained platform-dependent, so these results should not be interpreted as evidence of platform-agnostic deployment or equivalence with commercial PAM50-based assays (Prosigna). No claims of therapeutic equivalence or clinical decision-making impact are made.

\textbf{Reproducible derivation.} Fold assignment was deterministic (SHA-256 of patient identifiers), eliminating random seeds as a driver of gene selection and ensuring exact reproducibility from first principles. The gene count (N = 24) was not pre-specified; it was derived from the Pareto frontier of model complexity versus OOF C-index (Additional file~8: Figure~S6), where performance plateaus well before 50 genes.

\textbf{Gene selection stability.} Bootstrap resampling ($B = 200$ refits on SCAN-B) confirmed that 15 of 24 CorePAM genes were selected in at least 70\% of resamples (Additional file~16: Figure~S10), with 2 genes (ACTR3B, NAT1) selected in 100\%. The remaining genes show lower but non-negligible selection frequencies (minimum: 35.5\%), consistent with the elastic-net grouping property that distributes weight among correlated genes. Leave-one-gene-out analysis showed that no single gene removal causes catastrophic performance loss (Additional file~1: Figure~S1; maximum $|\Delta C|$ = 0.044).
...
\textbf{Calibration and transportability.} Calibration at clinically relevant horizons (60 months for SCAN-B, METABRIC, GSE20685; 24 months for TCGA-BRCA) showed adequate concordance between predicted and observed event rates in the training cohort (Figure~\ref{fig:calibration}). Formal logistic recalibration confirmed a near-ideal calibration slope in the training cohort ($\beta$ = 0.96; intercept $\alpha$ = 0.56, indicating moderate underprediction of absolute risk) with further miscalibration in external cohorts (slopes 1.57--2.74), consistent with model under-confidence when applying scores cross-platform without recalibration---a structural property of intra-cohort z-scoring. Exploratory decision curve analysis suggested positive net benefit across a range of probability thresholds (Additional file~10: Figure~S8).

\textbf{Frozen z-score sensitivity.} A key reviewer concern was whether the intra-cohort z-score normalisation---appropriate for retrospective multi-cohort validation---could support single-sample scoring. Our frozen z-score sensitivity analysis applied SCAN-B reference mean/SD per gene to each external cohort without any cohort-level re-standardisation, ensuring that each patient's score depends only on their own expression values and the frozen reference. Discrimination was preserved in TCGA-BRCA ($\Delta$C = +0.0002; $r$ = 0.999) and METABRIC ($\Delta$C = +0.0007; $r$ = 0.958). In the Affymetrix cohorts, frozen scoring showed measurable attenuation (GSE20685: $\Delta$C = $-$0.036, $r$ = 0.923; GSE1456: $\Delta$C = $-$0.040, $r$ = 0.916), consistent with the larger distributional gap between RNA-seq and these microarray platforms. These findings support platform-conditional rather than platform-agnostic fixed-reference scoring and indicate that platform-specific recalibration would be required before deployment.
...
\textbf{Comparison with established signatures.} An exploratory head-to-head comparison against research-based implementations (genefu R package~\cite{gendoo2016}) of the PAM50-derived Risk of Recurrence score (ROR-S, 50 genes) and the OncotypeDX Recurrence Score (ODX-RS, 21 genes) showed that CorePAM (24 genes) performs competitively (Additional file~17: Table~S7). CorePAM showed advantages in the RNA-seq cohorts (its training domain) and small deficits ($\Delta$C $< 0.03$) in the microarray cohorts (the development domain of those signatures). This comparison involves research-based score implementations---not commercial assays---and should be interpreted as evidence of competitive performance, not clinical equivalence.

\textbf{Clinical implications.} A smaller gene set may simplify future assay development, but analytical validation on specific deployment platforms and prospective clinical validation were beyond the scope of this study.

\textbf{Limitations.} \textit{Study design and data.} This is a retrospective study using public data, and no prospective validation was performed. TCGA-BRCA has structurally short follow-up (median 32 months), which may attenuate the detectable HR; the 24-month sensitivity analysis (Additional file~9: Figure~S7) partially mitigates this concern. METABRIC uses DSS as the primary endpoint, following established precedent for that dataset. Endpoint heterogeneity across cohorts is reflected in the moderate meta-analytic $I^2$ and should be considered when interpreting pooled estimates. HER2 status was not available in sufficient cohorts to permit HER2-stratified subgroup analysis.

\textit{Analytical choices.} The primary CORE-A clinical baseline was restricted to age and ER status---the two variables consistently available in harmonised form---although expanded models including T-stage and nodal status confirmed independent prognostic value (Additional file~3: Table~S1). ER status was unavailable in TCGA-BRCA and GSE20685, restricting CORE-A to age alone in those cohorts. The primary score formulation relied on cohort-wise z-score standardisation, which limits direct transportability of absolute risk estimates. In frozen-reference analyses, discrimination remained stable in TCGA-BRCA and METABRIC ($\Delta$C $<$ 0.001) but was attenuated in the Affymetrix cohorts (GSE20685: $\Delta$C = $-$0.036; GSE1456: $\Delta$C = $-$0.040), indicating that fixed-reference single-sample scoring is currently platform-dependent rather than platform-agnostic. Gene coverage remained high across validation cohorts (24/24 in METABRIC and GSE20685; 23/24 in TCGA-BRCA; 21/24 in GSE1456), and leave-one-gene-out sensitivity analysis confirmed that the score is robust to the absence of any single gene (Additional file~1: Figure~S1). Comparisons with ROR-S and ODX-RS used research-based implementations, not commercial assays.

\textit{Clinical translation.} A smaller gene set may simplify future assay development, but analytical validation on specific deployment platforms and platform-specific housekeeping-gene calibration were beyond the scope of this study and are the subject of planned technical validation work.
...
CorePAM is a reproducible 24-gene reduction of the PAM50 panel selected within a pre-specified $\Delta$C-index framework relative to a full 50-gene elastic-net comparator. Across four external RNA-seq and microarray cohorts, the reduced model retained prognostic discrimination. In cohorts with sufficient clinicopathological annotation, it also provided incremental value beyond a comprehensive six-variable clinical baseline (age, ER, T-stage, N-status, grade, HER2; LRT $p < 10^{-5}$), with the clinically meaningful signal concentrated in ER-positive disease. In direct paired external comparisons, discrimination was similar to or higher than the fuller comparator (Additional file~18). CorePAM was also associated with pathologic complete response in secondary neoadjuvant analyses ($I^2$ = 0\%). External calibration and frozen-reference performance remained platform-dependent, indicating that platform-specific recalibration and prospective analytical validation are required before single-sample deployment. The analytical pipeline is fully reproducible from publicly available data.
...
Additional files 1--18 are available as supplementary material. See the Additional files section at the end of this article for descriptions.
...
%% --- Additional file 1: Figure S1 — Gene dropout ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~1: Figure~S1.} Leave-one-gene-out sensitivity analysis. Change in C-index when each CorePAM gene is individually removed.}\label{fig:s1-dropout}
\end{figure}

%% --- Additional file 2: Figure S2 — METABRIC sensitivity ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~2: Figure~S2.} METABRIC sensitivity analyses. A: OS endpoint. B: Fine--Gray competing risks model. C: Quartile Kaplan--Meier (DSS).}\label{fig:s2-metabric-sens}
\end{figure}

%% --- Additional file 3: Table S1 — Expanded CORE-A ---
\begin{table}[!ht]
\caption{\textbf{Additional file~3: Table~S1.} Expanded CORE-A analysis: CorePAM independence from anatomical staging (T-stage, nodal status). HR per 1~SD CorePAM score from multivariable Cox regression.}\label{tab:s1-expanded}
...
GSE20685 & 327 & age+T+N & 1.45 (1.15--1.84) & 0.002 & 0.697 & 0.724 & +0.026 \\
\botrule
\end{tabular}
\end{table}

%% --- Additional file 4: Figure S3 — Correlation ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~4: Figure~S3.} Off-diagonal CorePAM score correlations between cohorts. Pearson $r$ computed on paired quantile values.}\label{fig:s3-correlation}
\end{figure}

%% --- Additional file 5: Figure S4 — PCA ---
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{additional_files/Additional_file_04_FigS4_PCA.png}
\caption{\textbf{Additional file~5: Figure~S4.} Forensic PCA of METABRIC expression matrix. Principal component analysis coloured by ER status.}\label{fig:s4-pca}
\end{figure}

%% --- Additional file 6: Figure S5 — Gene weights ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~6: Figure~S5.} CorePAM gene weights (lollipop plot). Elastic-net Cox regression coefficients. Positive weights (red): higher expression associated with worse survival.}\label{fig:s5-weights}
\end{figure}

%% --- Additional file 7: Table S2 — Gene weight list ---
\begin{table}[!ht]
\caption{\textbf{Additional file~7: Table~S2.} CorePAM gene list with elastic-net Cox regression weights (24 genes). Positive weights: higher expression associated with worse survival; negative weights: protective association.}\label{tab:s2-weights}
\begin{tabular}{@{}lrl@{}}
\toprule
...
%% --- Additional file 8: Figure S6 — Pareto ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~8: Figure~S6.} Pareto frontier: gene count vs.\ OOF C-index. Each point represents a unique df level along the glmnet regularisation path. Dashed red line: non-inferiority boundary ($C_{\text{max}} - \Delta C$; $\Delta C = 0.010$).}\label{fig:s6-pareto}
\end{figure}

%% --- Additional file 9: Figure S7 — TCGA 24-month ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~9: Figure~S7.} TCGA-BRCA 24-month sensitivity analysis.}\label{fig:s7-tcga-24m}
\end{figure}

%% --- Additional file 10: Figure S8 — DCA ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~10: Figure~S8.} Decision curve analysis (OS). Net benefit versus treat-all and treat-none strategies. Horizons: 60 months (SCAN-B, METABRIC, GSE20685) and 24 months (TCGA-BRCA).}\label{fig:s8-dca}
\end{figure}

%% --- Additional file 11: Figure S9 — CORE-A sensitivity ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~11: Figure~S9.} CORE-A baseline sensitivity analysis. Forest plot comparing CorePAM adjusted HR under different clinical baselines.}\label{fig:s9-corea-sens}
\end{figure}

%% --- Additional file 12: Table S3 — Frozen z-score ---
\begin{table}[!ht]
\caption{\textbf{Additional file~12: Table~S3.} Frozen z-score sensitivity analysis. C-index and Pearson correlation ($r$) between frozen and intra-cohort scoring approaches. \textbf{Note:} HRs in this table are per unit of raw weighted score (no cohort-level re-standardisation) and are therefore \textbf{not directly comparable} to the per-1-SD HRs in Table~\ref{tab:survival}. The relevant comparison metric is the C-index, which is rank-invariant across scoring scales.}\label{tab:s3-frozen}
\small
...
GSE20685 & 327 & Microarray & 0.591 & 0.626 & $-$0.036 & 0.923 \\
GSE1456 & 159 & Microarray & 0.622 & 0.661 & $-$0.040 & 0.916 \\
\botrule
\end{tabular}
...
%% --- Additional file 13: Table S4 — Uno's C + tdAUC ---
\begin{table}[!ht]
\caption{\textbf{Additional file~13: Table~S4.} Uno's C-statistic (IPCW) and time-dependent AUC at cohort-specific horizons.}\label{tab:s4-unoc}
...
%% --- Additional file 14: Table S5 — Formal calibration ---
\begin{table}[!ht]
\caption{\textbf{Additional file~14: Table~S5.} Formal calibration: logistic recalibration intercept ($\alpha$) and slope ($\beta$) at cohort-specific horizons. Ideal: $\alpha = 0$, $\beta = 1$.}\label{tab:s5-calibration}
...
%% --- Additional file 15: Table S6 — Full clinical baseline ---
\begin{table}[!ht]
\caption{\textbf{Additional file~15: Table~S6.} Full clinical baseline incremental value analysis. Cox regression with six clinical-pathological covariates (age, ER, T-stage, N-status, grade, HER2) $\pm$ CorePAM.}\label{tab:s6-fullbaseline}
\small
...
%% --- Additional file 16: Figure S10 — Bootstrap stability ---
\begin{figure}[!ht]
\centering
...
\caption{\textbf{Additional file~16: Figure~S10.} Bootstrap stability of CorePAM gene selection ($B = 200$). Frequency of each PAM50 gene across 200 bootstrap refits.}\label{fig:s10-bootstrap}
\end{figure}

%% --- Additional file 17: Table S7 — Genefu comparison ---
\begin{table}[!ht]
\caption{\textbf{Additional file~17: Table~S7.} Head-to-head C-index comparison: CorePAM (24 genes) vs research-based ROR-S (50 genes) and OncotypeDX-RS (21 genes). All scores per 1~SD.}\label{tab:s7-genefu}
...
%% --- Additional file 18: Table S8 — Head-to-head 24 vs 50 ---
\begin{table}[!ht]
\caption{\textbf{Additional file~18: Table~S8.} Head-to-head external comparison: CorePAM (24 genes) vs PAM50-full elastic-net (49 genes). Both models trained in SCAN-B under identical conditions (elastic-net Cox, $\alpha = 0.5$); CorePAM selected at the Pareto-optimal regularisation level, PAM50-full at the maximum-df $\lambda$. Scoring modes: intra-cohort z-score (framework-matched) and frozen z-score (deployment-matched, SCAN-B reference).}\label{tab:s8-headtohead}
\small
\begin{tabular}{@{}llcccccc@{}}
\toprule
Cohort & Mode & N & C$_{24}$ & C$_{49}$ & $\Delta$C & 95\% CI & $r$ \\
\midrule
TCGA-BRCA & intra & 1,072 & 0.624 & 0.631 & $-$0.008 & ($-$0.038, +0.022) & 0.853 \\
TCGA-BRCA & frozen & 1,072 & 0.624 & 0.634 & $-$0.010 & ($-$0.039, +0.020) & 0.864 \\
METABRIC & intra & 1,978 & 0.638 & 0.616 & +0.022 & (+0.011, +0.032) & 0.902 \\
METABRIC & frozen & 1,978 & 0.638 & 0.622 & +0.017 & (+0.007, +0.027) & 0.914 \\
GSE20685 & intra & 327 & 0.626 & 0.571 & +0.055 & (+0.010, +0.101) & 0.823 \\
GSE20685 & frozen & 327 & 0.591 & 0.544 & +0.047 & (+0.007, +0.085) & 0.843 \\
GSE1456 & intra & 159 & 0.661 & 0.564 & +0.097 & (+0.032, +0.166) & 0.697 \\
GSE1456 & frozen & 159 & 0.622 & 0.510 & +0.111 & (+0.045, +0.181) & 0.617 \\
\botrule
\end{tabular}
\footnotetext{$\Delta$C = C$_{24}$ $-$ C$_{49}$; positive values favour the 24-gene model. 95\% CI: bootstrap percentile (B = 2{,}000). $r$: Pearson correlation between the two linear predictors.}
\end{table}
