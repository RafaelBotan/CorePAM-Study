...
def wrap_label(text, width):
    """Wrap labels without splitting technical tokens inside didactic figures."""
...
# ============================================================================
# B2 — Subtipos intrinsecos e correlatos clinico-IHQ (footer FORA dos cards)
# ============================================================================
def fig_B2_subtipos_moleculares():
    # Figura alta com espaço reservado para rodapé abaixo dos cards
    fig, ax = plt.subplots(figsize=(13.2, 10.5))
    ax.set_xlim(0, 13.2); ax.set_ylim(0, 10.5)
    ax.axis("off")

    ax.text(6.6, 10.05, "Subtipos intrínsecos e correlatos clínicos do câncer de mama",
    ...
    subtypes = [
        dict(cx=3.3, cy=7.0, name="LUMINAL A", pct=40, color=C["navy"],
             note="PAM50: centroide luminal A",
             markers=["RE +", "RP +", "HER2 −", "Ki67 baixo"],
             prog="Excelente prognóstico",
             chemo="Baixa dependência de quimioterapia"),
        dict(cx=9.9, cy=7.0, name="LUMINAL B", pct=20, color=C["steel"],
             note="PAM50: centroide luminal B",
             markers=["RE +", "RP +/−", "HER2 −/+", "Ki67 alto"],
             prog="Prognóstico intermediário",
             chemo="Benefício variável de quimioterapia"),
        dict(cx=3.3, cy=3.0, name="HER2-ENRIQUECIDO", pct=15, color=C["graphite"],
             note="frequentemente, mas não sempre, HER2+",
             markers=["RE −/+", "RP −/+", "HER2 alto", "Ki67 alto"],
             prog="Maior agressividade sem alvo efetivo",
             chemo="Anti-HER2 se HER2 clínico positivo"),
        dict(cx=9.9, cy=3.0, name="BASAL-LIKE", pct=15, color=C["accent"],
             note="correlato usual: triplo-negativo",
             markers=["RE −", "RP −", "HER2 −", "CK5/6 ou EGFR +"],
             prog="Pior prognóstico inicial",
    ...
    for s in subtypes:
        ...
        ax.text(text_x, cy + h/2 - 0.48, s["name"],
                ha="left", va="center", fontsize=11.6,
                weight="bold", color=s["color"], zorder=5)
        ax.text(text_x, cy + h/2 - 0.76, s["note"],
                ha="left", va="center", fontsize=7.4,
                style="italic", color=C["stone"], zorder=5)
        # Fina linha sob o nome
        ax.plot([text_x, text_x + 2.15], [cy + h/2 - 0.62, cy + h/2 - 0.62],
                color=s["color"], lw=0.8, zorder=5)

        # Correlatos imuno-histoquimicos usuais (2 colunas compactas)
        mk_y = cy + 0.18
        ax.text(text_x, mk_y + 0.46, "Correlato IHQ usual",
                ha="left", fontsize=7.7, style="italic", color=C["stone"])
        ax.text(text_x, mk_y,
        ...
        prog = wrap_label(s["prog"], 24)
        prog_lines = prog.count("\n") + 1
        ax.text(text_x, cy - h/2 + 0.72, prog,
                ha="left", va="top", fontsize=9.4, linespacing=1.04,
                weight="bold", color=s["color"])
        chemo_y = cy - h/2 + (0.32 if prog_lines == 1 else 0.23)
        ax.text(text_x, chemo_y, wrap_label(s["chemo"], 46),
                ha="left", va="top", fontsize=7.8, color=C["stone"],
                style="italic")

    # Rodapé BEM ABAIXO dos cards (y=0.6, cards terminam em y=1.4)
    ax.text(6.6, 0.82,
            "Leitura correta: PAM50 classifica por centroides de expressão; "
            "RE/RP/HER2/Ki67 são correlatos clínicos aproximados.",
            ha="center", fontsize=8.5, style="italic", color=C["stone"])
    ax.text(6.6, 0.50,
...
def fig_B3_timeline_assinaturas():
    ...
    ax.set_xlim(1998.2, 2029.8); ax.set_ylim(-4.9, 6.1)
    ax.axis("off")
    ...
    for yr, name, n, plat, purpose, col, side in sigs:
        ...
        ax.text(card_x + card_w/2, card_y + 0.25,
                wrap_label(purpose, 25), ha="center", va="center",
                fontsize=8.6, linespacing=1.0,
...
def fig_B6_consort_coortes():
    ...
    ax.text(6.5, 10.65, "Fluxograma das coortes do estudo",
            ha="center", fontsize=16, weight="bold", color=C["ink"])
    ax.text(6.5, 10.25,
            "10 coortes públicas — 5 para validação prognóstica (sobrevida) "
