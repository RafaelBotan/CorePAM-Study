...
We thank the Editors and Reviewers for their thorough and constructive evaluation. We revised the manuscript extensively and addressed each concern with additional analyses, broader validation, and clearer reporting. Below, we respond point-by-point to each comment.
...
\begin{itemize}
  \item \textbf{Training cohort:} SCAN-B (GSE96058; N=3,069; RNA-seq), one of the largest publicly available RNA-seq breast cancer cohorts with matched clinical follow-up.
...
This approach eliminates the subjectivity flagged by the reviewer and replaces it with a transparent, reproducible, data-driven framework. Cross-platform reproducibility was evaluated by out-of-cohort discrimination across RNA-seq and microarray validation cohorts, score concordance analyses (off-diagonal Pearson correlations), and a frozen-reference scoring sensitivity analysis (Additional file~14: Table~S4).
...
We have added a head-to-head comparison with the research-based ROR-S (50 genes; centroid-based) and OncotypeDX-RS (21 genes) implementations from the \texttt{genefu} R package (Additional file~13: Table~S3), which provides the comparative context the reviewer seeks --- albeit at the score level rather than categorical subtype level. Additionally, PAM50 intrinsic subtype frequencies per cohort (using the standard centroid-based research implementation) are reported descriptively in Table~2, providing a sanity check of population composition across datasets.
...
We thank the reviewer for this careful reading. The revised manuscript now uses \textbf{nine external cohorts} (four for OS/DSS prognosis, four for pCR prediction, and one exploratory pCR cohort), and all cohort counts are explicitly defined. The cohort architecture is:
...
\begin{enumerate}
  ...
  \item \textbf{Meta-analysis explicitly separates endpoint types:} The primary meta-analysis (K=4) pools OS (TCGA-BRCA, GSE20685, GSE1456) and DSS (METABRIC) under the umbrella of ``time-to-event survival endpoint''; this is justified because both measure cancer-related mortality risk, and DSS is a more specific subset of OS. A harmonised OS-only sensitivity analysis (K=3, substituting METABRIC OS for DSS) is reported separately and confirms consistent results.
...
Frozen z-score & Not assessed & Preserved discrimination ($r = 0.921$--$1.000$) \\
