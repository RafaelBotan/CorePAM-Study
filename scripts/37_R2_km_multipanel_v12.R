# =============================================================================
# SCRIPT: 37_R2_km_multipanel_v12.R
...
# NOTE: thesis v1.2 rendering copy. It writes *_v12 outputs so the accepted
# manuscript/thesis v1.1 figure files are not overwritten.
# =============================================================================

source("scripts/00_setup.R")
source("scripts/00_colors.R")
SCRIPT_NAME <- "37_R2_km_multipanel_v12.R"
...
build_all_panels <- function(lang) {
  ...
  lr_p <- pchisq(lr_test$chisq, df = 1, lower.tail = FALSE)  # robust: avoids 1 - 1 underflow to 0 (F7 fix)
...
for (lang in c("EN", "PT")) {
  ...
  suffix <- lang
  out_pdf_main <- file.path(pdf_dir, sprintf("Fig2_KM_MultiPanel_%s_v12.pdf", suffix))
  out_png_main <- file.path(png_dir, sprintf("Fig2_KM_MultiPanel_%s_v12.png", suffix))
  ...
  if (lang == "EN") {
    out_png_sub <- file.path(ROOT_REPO, "submission", "figures", "Fig2_KM_MultiPanel_EN_v12.png")
    file.copy(out_png_main, out_png_sub, overwrite = TRUE)
    message(sprintf("[%s] Copied to: %s", SCRIPT_NAME, out_png_sub))
    registry_append("ALL", "figure_km_multipanel_R2_v12", out_pdf_main,
